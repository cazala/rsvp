      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {[
            ...
            { label: "Restricciones Alimentarias", value: dietary, icon: Calendar },
                  ...
                  <tbody>
                    {rsvps.map((r) => (
                      <tr key={r.id} className="border-b hover:bg-gray-50 group">
                        ...
                        <td className="p-2">{r.email ?? <span className="italic text-gray-400">–</span>}</td>
                        <td className="p-2">{r.whatsapp ?? <span className="italic text-gray-400">–</span>}</td>
                        <td className="p-2">
                          <span
                            className={`px-2 py-1 rounded-full text-xs ${
                              r.is_minor ? "bg-orange-100 text-orange-800" : "bg-gray-100 text-gray-800"
                            }`}
                          >
                            {r.is_minor ? "Sí" : "No"}
                          </span>
                        ...
                        <td className="p-2">
                          <span
                            className={`px-2 py-1 rounded-full text-xs ${
                              r.needs_transfer ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
                            }`}
                          >
                            {r.needs_transfer ? "Sí" : "No"}
                          </span>
                        </td>
                        <td className="p-2 max-w-xs truncate" title={r.dietary_requirements ?? ""}>
                          {r.dietary_requirements || "-"}
                        </td>
                        <td className="p-2 max-w-xs truncate" title={r.comment ?? ""}>
                          {r.comment || "-"}
                        </td>
                        <td className="p-2">{new Date(r.created_at).toLocaleDateString("es-AR")}</td>
                        <td className="p-2">
